...
Reorganized the `images/` directory from a flat 32-file structure to organized subdirectories:

**Before:**
```
images/
├── boss_bb.svg (and 8 more bosses)
├── bullet_enemy.svg
├── bullet_player.svg
├── enemy.svg
├── player.svg
...
```
images/
├── bosses/ (9 boss sprites)
├── bullets/ (2 bullet sprites)
├── entities/ (2 entity sprites: player, enemy)
└── misc/ (3 miscellaneous files)
```

### Documentation Reorganization

Created `docs/` directory and moved documentation files:
- CONSOLIDATION_REPORT.md → `docs/`
- VALIDATION_CHECKLIST.md → `docs/`
- REFACTORING_SUMMARY.md → `docs/`

Kept `readme.md` at project root (standard convention).
...
- Asset directories (`assets/` and `assets2/`) left unchanged as both are actively used
- Shader files already well-organized (6 shaders with .uid files)
