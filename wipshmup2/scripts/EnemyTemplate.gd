...
func create_enemy_instance() -> Node:
	...
	var movement_behavior = movement_script.new()
	movement_behavior.name = "MovementBehavior"
	enemy.add_child(movement_behavior)
	
	# Apply movement parameters
	for key in movement_params:
		if movement_behavior.has_method("set_" + key):
			movement_behavior.call("set_" + key, movement_params[key])
		elif movement_behavior.get(key) != null:
			movement_behavior.set(key, movement_params[key])
	
	# Add attack behavior
	var attack_script = get_attack_behavior_scene()
	var attack_behavior = attack_script.new()
	attack_behavior.name = "AttackBehavior"
	enemy.add_child(attack_behavior)
	
	# Apply attack parameters
	for key in attack_params:
		if attack_behavior.has_method("set_" + key):
			attack_behavior.call("set_" + key, attack_params[key])
		elif attack_behavior.get(key) != null:
			attack_behavior.set(key, attack_params[key])
