...
func _add_movement_behavior() -> void:
	...
	movement_behavior = behavior_script.new()
	movement_behavior.name = "MovementBehavior"
	boss.add_child(movement_behavior)
	
	# Apply movement parameters
	for key in current_phase.movement_params:
		if movement_behavior.has_method("set_" + key):
			movement_behavior.call("set_" + key, current_phase.movement_params[key])
		elif movement_behavior.get(key) != null:
			movement_behavior.set(key, current_phase.movement_params[key])
